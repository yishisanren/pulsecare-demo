...
### 双模式日程当天的衔接
当天属于双模式日程（08+09 或 06+07）时，训练完成激励按"单次训练"独立触发：
- 完成第 1 个任务（如 06 Chronic）后正常显示激励页 + 心情反馈，关闭后回到日程主页
- 主页可见第 2 个任务卡片（07 Acute）仍为 Pending，可立即开始或稍后训练
- 2 个任务都完成后，整条双模式日程当天才记 +1（共用进度条）
- 日程**周期完成**的庆祝页要等两个模式都跑完才触发（详见日程完成页）
